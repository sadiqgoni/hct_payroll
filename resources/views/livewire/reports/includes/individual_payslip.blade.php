@forelse($payslips as $paySlip)
    <?php
        $loan = [];
        if (!empty($paySlip->balances)) {
            $loan = json_decode($paySlip->balances, true);
        }
        if (!is_array($loan)) {
            $loan = [];
        }
    ...
    <table style="margin: auto;width: 95%">
        <tbody>
            ...
            <tr>
                ...
                <td style="padding-left: 50px">Pension Pin: {{$paySlip->pfa_name}} <br>Bank Name: {{$paySlip->bank_name}}
                </td>
    ...
    <table style="width: 75%;margin-left: 3%;font-size: 13px !important;">
        ...
        <tbody>
            ...
            <tr>
                <td style="font-weight: bolder" colspan="4"><b>Total Ded:{{number_format($paySlip->total_deduction, 2)}}</b>
                </td>
            </tr>
            <tr>
