            ...
            <div class="col table-responsive">
                            ...
                            <div class="col-12 col-md-6 col-lg4 offset-md-6 offset-lg-6">
                                <fieldset class="px-4">
                                    <legend>
                                        ...
                                        @if($edit==true)
                                            <h6> Update Staff Category </h6>
                                        @endif
                                        @if($create==true)
                                            <h6> Add Staff Category </h6>
                                        @endif
                                    </legend>
                                    <form class="">


                                        <input type="text" wire:model="name" class="form-control " placeholder="Staff Category">
                                        @if($create==true)
                ...
                <table class="table table-striped table-bordered table-sm">
                    ...
                    @forelse($posts as $post)
                        ...
                        <tr>
                            <th>{{($posts->currentPage() - 1) * $posts->perPage()+1 + $loop->index+1}}</th>
                            <td>{{$post->name}}</td>
                            <td>
                                @if($post->status==1)
