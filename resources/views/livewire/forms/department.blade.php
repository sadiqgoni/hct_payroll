        ...
        <div class="col table-responsive">
                    ...
                    <div class="col-12 col-md-6 col-lg4 offset-md-6 offset-lg-6">
                        <fieldset class="px-4">
                            ...
                            <form class="">
                                ...
                                <select name="" wire:model="unit" id="" class="form-control mb-2 @error('unit') is-invalid @enderror">
                                    <option value="">Select Unit</option>
                                    @foreach(\App\Models\Unit::where('status',1)->get() as $unit)
            ...
            <table class="table table-striped table-bordered table-sm">
                ...
                @forelse($posts as $post)
                    ...
                    <tr>
                        <th>{{($posts->currentPage() - 1) * $posts->perPage() + $loop->iteration}}</th>
                        <td>{{$post->name}}</td>
                        <td>{{unit_name($post->unit_id)}}</td>
                        <td>
                            @if($post->status==1)
