        ...
        <div class="">
            <div class="text-right">
                ...
                <select name="" id="" class="form-control-sm float-left ml-3" wire:model.live="perpage">
                    ...
                    @can('can_save')
                    <button class="btn mt-2 create" wire:click.prevent="create_deduction()">Add</button>
                @endcan
            </div>
            ...
            <table class="table mt-2 table-striped table-bordered">
                ...
                @forelse($deductions as $deduction)
                    <tr>
                        <th>{{($deductions->currentpage() -1 ) * $deductions->perPage() + $loop->iteration}}</th>
                        <td>{{ss($deduction->salary_structure_id)}}</td>
                        <td>{{$deduction->grade_level_from}}</td>
                        <td>{{$deduction->grade_level_to}}</td>
                        <td>{{deduction_name($deduction->deduction_id)}}</td>
                        <td>{{ded_type($deduction->deduction_type)}}</td>
                        <td>@if($deduction->deduction_type==1)
                                {{$deduction->value}}%
                            @else
                        ...
                        <td>
                            @can('can_edit')
                                <button class="btn edit_btn" wire:click.prevent="edit_deduction({{$deduction->id}})" style="width:fit-content !important;padding: 5px !important;">Edit</button>
                                <button class="btn btn-sm btn-danger" wire:click.prevent="deleteId({{$deduction->id}})" style="width:fit-content !important;padding: 5px !important;">Delete</button>
                ...
                <tfoot>
                <tr>
                    <td colspan="5">{{$deductions->links()}}</td>
                </tr>
                </tfoot>
            </table>
            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                @php
                                    $salObj=\App\Models\SalaryStructure::find($this->salary_structure);
                                @endphp
                                <label for="">Grade Level To  @error('grade_level')<small class="text-danger">{{$message}}</small>@enderror</label>
                            ...
                            <div class="col-12 col-md-4">
                                <div class="form-group">
                                    ...
                                    @foreach(\App\Models\Deduction::all() as $ss)
                                        <option value="{{$ss->id}}">{{$ss->deduction_name}}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-12 col-md-4">
                                <div class="form-group">
                                    <label for="">Deduction Type   @error('deduction_type')<small class="text-danger d-block form-text">{{$message}}</small>@enderror</label>

                                    <select name="" id="" wire:model.defer="deduction_type" class="form-control">
                    ...
                    <fieldset>
                        ...
                        <div class="row">
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    <label for="">Salary Structure @error('salary_structure') <small class="text-danger">{{$message}}</small> @enderror</label>
                                    <select name="" class="form-control text-capitalize" wire:model.live="salary_structure">
                                        <option value="">Select Salary Structure</option>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="form-group">
                                    <label for="">Deduction @error('deduction') <small class="text-danger">{{$message}}</small> @enderror</label>
                                    <select name="" class="form-control text-capitalize" wire:model.defer="deduction">
                                        <option value="">Select Deduction</option>
                                        @foreach(\App\Models\Deduction::all() as $ss)
                                            <option value="{{$ss->id}}">{{$ss->deduction_name}}</option>
                                        @endforeach
...
@section('title')
        Salary Deduction Template
    @endsection
    @section('page_title')
        Payroll Settings / Deduction Template
    @endsection
</div>
